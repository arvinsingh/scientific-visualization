	...
	*/
	boolean ReadData(String fileName, int sizeX, int sizeY, int sizeZ, int headerSize)
	{
	    ...
	    try
		{
			...
			//Copying the byte values into the integer array
...
public class Slice
{
	...
	public static void SaveImageRGB(String name, int[][][] im)
	{
		...
		try 
		{
			System.out.println("Saving image " + name + " RGB ...");
			ImageIO.write(image, "png", f);
		} 
		catch (IOException e) 
		{
	...
	public static void main(String[] args) 
	{
		...
		if (args.length < 1) {
            System.out.println("Usage: java Skull <isovalue>");
            return;
        }
		...
		System.out.println("Extracting slice " + Integer.parseInt(args[0]) + " ...");
		SaveImage("im_" + Integer.parseInt(args[0]) + ".png",v.ExtractSlice(Integer.parseInt(args[0])));
		SaveImage("imGradientMagnitude_" + Integer.parseInt(args[0]) + ".png",v.ExtractSliceOfGradientMagnitude(gradient,Integer.parseInt(args[0])));
		SaveImageRGB("imGradient_" + Integer.parseInt(args[0]) + ".png",v.ExtractSliceOfGradient(gradient,Integer.parseInt(args[0])));
	}
}